...
const MSP_ID = 'IBMSP'; // Le portail agit en tant qu'HU
const TEST_NETWORK_HOME = path.resolve(__dirname, '..', 'test-network');
const CRYPTO_PATH = path.join(TEST_NETWORK_HOME, 'organizations', 'peerOrganizations', 'ib.example.com');

const KEY_DIRECTORY_PATH = path.join(CRYPTO_PATH, 'users', 'User1@ib.example.com', 'msp', 'keystore');
const CERT_DIRECTORY_PATH = path.join(CRYPTO_PATH, 'users', 'User1@ib.example.com', 'msp', 'signcerts');
const TLS_CERT_PATH = path.join(CRYPTO_PATH, 'peers', 'peer0.ib.example.com', 'tls', 'ca.crt');

const PEER_ENDPOINT = 'localhost:9051';
const PEER_HOST_ALIAS = 'peer0.ib.example.com';
