...
//
// Le relais agit avec l'identite MSP d'CGNMSP. Ce choix est arbitraire entre
// CGNMSP et IBMSP : les deux organisations sont membres de global-channel
...
// sur project-channel. HUMSP ne pourrait pas jouer ce role car elle n'est
...
const CRYPTO_PATH = path.resolve(
    ...
    'organizations', 'peerOrganizations', 'cgn.example.com',
);

const KEY_DIRECTORY_PATH = path.resolve(
    CRYPTO_PATH, 'users', 'User1@cgn.example.com', 'msp', 'keystore',
);

const CERT_DIRECTORY_PATH = path.resolve(
    CRYPTO_PATH, 'users', 'User1@cgn.example.com', 'msp', 'signcerts',
);

const TLS_CERT_PATH = path.resolve(
    CRYPTO_PATH, 'peers', 'peer0.cgn.example.com', 'tls', 'ca.crt',
);

// Adresse du peer gRPC d'CGN, par defaut celle de test-network.
const PEER_ENDPOINT = process.env.PEER_ENDPOINT || 'localhost:7051';
const PEER_HOST_ALIAS = process.env.PEER_HOST_ALIAS || 'peer0.cgn.example.com';

/**
 * Cree une connexion gRPC TLS vers le peer d'CGN.
 * Cette connexion unique est reutilisee pour les deux canaux
 * (global-channel et project-channel), puisque le meme peer CGN
 * heberge les deux.
 */
